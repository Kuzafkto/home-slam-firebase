...
export class PlayersService {
  ...
  public addplayer(player: Player): Observable<Player> {
    ...
    let player_mapped = {
      data: {
        ...
        age: player.age,
        positions: {
          set: player.positions //set establece la relacion del player con las positions indicando los id de la tabla position que tiene el array positions de player
        }
      }
    };
      ...
      switchMap(res => {
        ...
        return this.api.get('/players?populate=positions').pipe(
          map(response => {
            ...
            transformedPlayers.sort((a: Player, b: Player) => b.id - a.id);
  ...
  */
  ...
  public updateplayer(player: Player): Observable<Player> {
    return new Observable<Player>(obs => {
      if (player && player.id) {
        ...
        this.api.put(`/players/${player.id}`, payload).subscribe(_ => {
  ...
  public deleteplayer(player: Player): Observable<Player> {
    return new Observable<Player>(obs => {
      this.api.delete(`/players/${player.id}`).subscribe(_ => {
        this.getAll().subscribe(_ => {
          obs.next(player);
        })
      })
